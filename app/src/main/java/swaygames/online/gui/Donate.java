...
import swaygames.online.gui.adapters.ActionsAdapter;
...
public class Donate extends AppCompatActivity {
    ...
    public Donate(Activity aactivity) {
        ...
        actija = activity.findViewById(R.id.textView3);
...
// donateList.add(new Donatee(11, "", "", "auto_", " BC"));
    ...
    public void show_actions() {
       ...
       /* Retrofit retrofit = new Retrofit.Builder()
        ...
        Call<List<Actions>> ncall = sInterface.getActions();

        ncall.enqueue(new Callback<List<Actions>>() {
            @Override
            public void onResponse(Call<List<Actions>> call, Response<List<Actions>> response) {

                List<Actions> news = response.body();

                for (Actions storie : news) {
                    Lists.nlist.add(new Actions(action.getImageUrl(), storie.getTitle(), storie.getUrl()));
                }
            }
